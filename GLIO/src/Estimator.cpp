...
private:
    ...
    */
    ...
    double timeshift_IMUtoGNSS, gnssweek_gap;
...
public:
    Estimator(): nh("~") {
        ...
        if (data_set == "HK") {
            span_BP_sub =nh.subscribe("/novatel_data/inspvax_10hz", 500, &Estimator::span_bp_callback, this);
        }
    ...
    void initializeParameters() {
        ...
        if (!getParameter("/initialization/gnssweek_gap", gnssweek_gap)) {
            ROS_WARN("data_set not set, use default value: 0");
            gnssweek_gap = 0;
        }
...
//        res_lc_evo_output.close();
    ...
    */
    ...
    void rtklib_ref_gnss_meas_callback(const nlosExclusion::GNSS_Raw_ArrayConstPtr &gnss_msg)
    {
        ...
        if(true) //disNorm< 30
        {
            gtime_t gps_time_ = gpst2time(gnss_msg->GNSS_Raws[0].GNSS_week + gnssweek_gap, gnss_msg->GNSS_Raws[0].GNSS_time);
    ...
    */
    ...
    void rtklibOdomHandler(const nav_msgs::Odometry::ConstPtr& odomIn) {
        if(RTKinLocal)
        {
            ...
            if (rtk_ini_enu_path.poses.size() == 0) {
                double initial_dist = (ECEF - anc_ecef).norm();
                if (initial_dist > 100) {
                    anc_ecef = ECEF;
                    ...
                    cout << "initialize anc_ecef: " << endl << anc_ecef << endl;
                }
            }

            Eigen::Matrix<double, 3,1> ENU_ref = m_GNSS_Tools.ecef2llh(anc_ecef);
    ...
    void rtklib_gnss_meas_callback(const nlosExclusion::GNSS_Raw_ArrayConstPtr &meas_msg)
    {
        ...
        cout << "max_diff init: " << max_diff << " station[0] time: " << iter_pr->first << " cur time: " << integerGNSSTime << endl;
        ...
        cout << "st_gnss_data size: " <<st_gnss_data.GNSS_Raws.size() << " epochStGNSSMap size: " << epochStGNSSMap.size() << endl;
        ...
        */
        ...
        cout << "gps_time: " << gps_time << " length: " << length
        << " station_gnss_raw_map size: " << station_gnss_raw_map.size()
        << " epochStGNSSMap size: " << epochStGNSSMap.size() << endl;
        /* index the rcv gnss */
        for (size_t i = 0; i < length; i++)
        {
            ...
            if (satPrn > 160) {
                cout << "satPrn larger than 160" << endl;
                continue;
            }
            if(!epochStGNSSMap.count(satPrn)) {
                cout << "station not count sat" << endl;
                continue;
            }
        ...
        cout << "meas_msg_valid size: " << meas_msg_valid.GNSS_Raws.size() << endl;
    ...
    bool addDDPsrResFactor_gl(ceres::Problem& problem, nlosExclusion::GNSS_Raw_Array user_gnss_data, nlosExclusion::GNSS_Raw_Array ref_gnss_data, int mPrn, MatrixXd weight_matrix, int leftKey, int rightKey, double ts_ratio, double DDpsrThreshold)
    {
        ...
        cout << setprecision(12);
        cout << "user_gnss_data size: "  << user_gnss_data.GNSS_Raws.size() << " ref_gnss_data size: " << ref_gnss_data.GNSS_Raws.size() << endl;
        cout  << "mPrn: " << mPrn << endl;
        for (int i=0; i<user_gnss_data.GNSS_Raws.size(); i++) {
            cout << "time: " << user_gnss_data.GNSS_Raws[i].GNSS_time << " " << ref_gnss_data.GNSS_Raws[i].GNSS_time << endl;
            cout << "prn: " << user_gnss_data.GNSS_Raws[i].prn_satellites_index << " " << ref_gnss_data.GNSS_Raws[i].prn_satellites_index << endl;
            cout << "psr: " << user_gnss_data.GNSS_Raws[i].raw_pseudorange << " " << ref_gnss_data.GNSS_Raws[i].raw_pseudorange << endl;
            cout << "pos: " << user_gnss_data.GNSS_Raws[i].sat_pos_x << " "
            << user_gnss_data.GNSS_Raws[i].sat_pos_y << " "
            << user_gnss_data.GNSS_Raws[i].sat_pos_z << endl;
            cout << "pos ref: " << ref_gnss_data.GNSS_Raws[i].sat_pos_x << " "
            << ref_gnss_data.GNSS_Raws[i].sat_pos_y << " "
            << ref_gnss_data.GNSS_Raws[i].sat_pos_z << endl;
        }

#if 1
//        std::cout << "finish add L1 psr residual" << std::endl;
        double **parameters_evo;
        double *residuals_evo = new double;
        double **jacobians_evo;

        std::vector<int> block_sizes = dd_psr_factor->parameter_block_sizes();
        std::vector <Eigen::VectorXd> parameters_eigen, jacobians_eigen;
        parameters_evo = new double *[block_sizes.size()];
        jacobians_evo = new double *[block_sizes.size()];
        parameters_eigen.resize(block_sizes.size());
        jacobians_eigen.resize(block_sizes.size());
        ...
        for (int l = 0; l < static_cast<int>(block_sizes.size()); l++) {
            ...
            jacobians_eigen[l].resize(19 * block_sizes[l]);
            jacobians_evo[l] = jacobians_eigen[l].data();
        }

        Eigen::VectorXd residuals_evo_size;
        residuals_evo_size.resize(20 - 1);
        ...
        parameters_evo[2][0] = para_yaw_enu_local[0]; // yaw difference between enu and local
        parameters_evo[3][0] = para_anc_ecef[0];
        parameters_evo[3][1] = para_anc_ecef[1];
        parameters_evo[3][2] = para_anc_ecef[2]; // anchor point

        dd_psr_factor->Evaluate(parameters_evo, residuals_evo, jacobians_evo);
        std::vector<double> residuals_evo_values(residuals_evo, residuals_evo + 20 - 1);
//        std::cout << "finish addtestDDPsrResFactor" << std::endl;
        std::cout << "l1 residuals_ddpsr_values: ";
        cout << setprecision(4);
        for (int i=0; i<user_gnss_data.GNSS_Raws.size(); i++) {
            std::cout << residuals_evo_values[i] << " ";
        }
        cout << endl;
//        int j = 0;
//        for (int i=0; i<user_gnss_data.GNSS_Raws.size()-1; i++) {
//            avg_psr1 += fabs(residuals_evo_values[i]);
//            int prn = int(user_gnss_data.GNSS_Raws[j].prn_satellites_index);
//            if (prn == mPrn) j++;
//            prn = int(user_gnss_data.GNSS_Raws[j].prn_satellites_index);
//            std::string spsr_filename =
//                    data_path + "overall_info/batch/batch_details/" + std::to_string(prn) +
//                    "_batch_psr.csv";
//            FILE *prn_batch_psr = fopen(spsr_filename.c_str(), "a+");
//            gtime_t gps_time_ = gpst2time(user_gnss_data.GNSS_Raws[j].GNSS_week, user_gnss_data.GNSS_Raws[j].GNSS_time);
//            double obs_local_ts = time2sec(gps_time_) - timeshift_IMUtoGNSS;
//            fprintf(prn_batch_psr, "%7.3f, %7.3f \n", obs_local_ts, residuals_evo_values[i]);
//            fflush(prn_batch_psr);
//            j++;
//        }
//        avg_psr1 /= residuals_evo_values.size();
////        for (int l = 0; l < static_cast<int>(block_sizes.size()); l++) {
////            std::cout << "jacobian: " << std::endl;
////            for (int j=0; j<19 * block_sizes[l]; j++) {
////                std::cout << jacobians_evo[l][j] << " ";
////            }
////            std::cout << std::endl;
////        }
#endif
...
//            cout << "ba: " << tmpSpeedBias[i-keyframe_idx[keyframe_idx.size()-slide_window_width]][3] << " "
//            << tmpSpeedBias[i-keyframe_idx[keyframe_idx.size()-slide_window_width]][4] << " " <<
//            tmpSpeedBias[i-keyframe_idx[keyframe_idx.size()-slide_window_width]][5] << endl;
//            cout << "bg: " << tmpSpeedBias[i-keyframe_idx[keyframe_idx.size()-slide_window_width]][6] << " "
//                 << tmpSpeedBias[i-keyframe_idx[keyframe_idx.size()-slide_window_width]][7] << " " <<
//                 tmpSpeedBias[i-keyframe_idx[keyframe_idx.size()-slide_window_width]][8] << endl;
...
//        Timer t_optimize_batch("optimizeBatchWithLandMark");
        *pose_info_keyframe_batch = *pose_info_keyframe; //pose of each frame
        ...
        int iteration_num = 1;
        double DDpsr_threshold[iteration_num] = {1000000000}; // 1000000000, 10, 8, 6, 4, 3, 2.5
...
//                        continue;
                ...
                ref_gnss_data = ref_gnss_meas_vec_tmp[i];
                prepareGLODDPsrData(gnss_data, ref_gnss_data, mPrn);
                if(gnss_data.GNSS_Raws.size()>2 && 0)
                {
                    getDMatrix(gnss_data, mPrn, Dmatrix);
                    Eigen::MatrixXd subweight_matrix;
                    subweight_matrix = m_GNSS_Tools.cofactorMatrixCal_WLS(gnss_data, "WLS"); //goGPS cofactorMatrixCal_WLS
...
//                }
            ...
            if (iterCount == iteration_num - 1) {
                ...
                cout << "fist postition: " << pose_info_keyframe_batch->points[0].x 
                << " " << pose_info_keyframe_batch->points[0].y 
                << " " << pose_info_keyframe_batch->points[0].z << endl;
...
//                    fout_batch_evo.close();
                    ...
                    Eigen::Vector3d batch_vel_i (gl_tmpSpeedBias[i][0], gl_tmpSpeedBias[i][1], gl_tmpSpeedBias[i][2]);
